...
public class Console {
	 ...
	 *
	 * @param message
	 * 		Message to show to the user as to what you are requesting
	 *
	 ...
	 * 		Message to show to the user as to what you are requesting
	 * @param textColor
	 * 		ConsoleTextColor of the text when written
	 *
	 * @return Returns a char value representing the users input
	 */
	 ...
	 * 		Message to show to the user as to what you are requesting
	 * @param positive
	 * 		The positive value to ask the user for (e.g. "Yes")
	 * @param negative
	 * 		The negative value to ask the user for (e.g. "No");
	 *
	 ...
	 * 		Message to show to the user as to what you are requesting
	 * @param min
	 * 		Minimum allowed short value
	 * @param max
	 * 		Max allowed short value
	 * @param textColor
	 * 		ConsoleTextColor of the text when written
	 *
	 * @return Returns a short value representing the users input
	 ...
	 * 		Message to show to the user as to what you are requesting
	 * @param min
	 * 		Miniumum allowed long value
	 * @param max
	 * 		Maximum allowed long value
	 * @param textColor
	 * 		ConsoleTextColor of the text when written
	 *
	 * @return Returns a long value representing the users input
	 ...
	 * 		Message to show to the user as to what you are requesting
	 * @param min
	 * 		Minimum number the user can provide
	 * @param max
	 * 		Maximum number the user can provide
	 *
	 * @return Returns an int value representing the users input
	 */
	 ...
	 * 		Message to show to the user as to what you are requesting
	 * @param allowEmpty
	 * 		If the method should allow empty (e.g. enter key or space)
	 *
	 * @return The String the user typed meeting the requirements
	 */
	 ...
	 * 		Message to show to the user as to what you are requesting
	 * @param minCharacters
	 * 		The minimum amount of characters allowed in this string.
	 * @param maxCharacters
	 * 		The maximum amount of characters allowed in this string.
	 * @param textColor
	 * 		ConsoleTextColor of the text when written
	 *
	 * @return The String the user typed meeting the requirements
	...
	public static String getStringInput(String message, int minCharacters, int maxCharacters, TextColor textColor) {
		...
		do {
			...
			if (response.length() < minCharacters || response.length() > maxCharacters) {
				Console.writeln(
						"Your input must be between " + minCharacters + " and " + maxCharacters + " characters in length!",
						TextColor.RED
				);
				response = null;
			}
	 ...
	 *
	 * @param message
	 * 		The message to show the user under the Options list.
	 * @param enumList
	 * 		The list of possible enumerations to show the user. Use the EnumName.values() method to get this.
	 * @param listColor
	 * 		The color of the options list values
	 * @param messageColor
	 * 		The color of the message under the options list
	 * @param allowNone
	 * 		If you allow the user to pick a "None" option to choose none of them.
	 * @param <T>
	 * 		This is a generic Enum type. Can be any Enumeration value.
	 *
	 * @return Returns the selected enum value, or null if the "None" option was chosen
	 */
	public static <T extends Enum<T>> T getEnumValueAsMenu(String message, T[] enumList, TextColor listColor,
														   TextColor messageColor, boolean allowNone
	) {
	 ...
	 * 		The list of possible enumerations to show the user. Use the EnumName.values() method to get this.
	 * @param allowEmpty
	 * 		If you allow the user to just hit entire and not pick one. If false, they are asked until they give a valid answer.
	 * @param listColor
	 * 		The color of the options list values
	 * @param messageColor
	 * 		The color of the message under the options list
	 ...
	 */
	public static <T extends Enum<T>> T getEnumValueAsList(String message, T[] enumList, boolean allowEmpty, TextColor listColor,
														   TextColor messageColor
	) {
		if (enumList != null && enumList.length > 0 && enumList[0] != null) {
			do {
				String enumOptions = "Options: " + String.join(", ", java.util.Arrays.stream(enumList).map(Enum::name).map(
						name -> name.replace("_", " ")).toArray(String[]::new)
				);
	...
	public static LocalDate getDateInput(String message, LocalDate minDate, LocalDate maxDate, TextColor textColor) {
		do {
			...
			try {
				...
				Console.writeln(String.format("Invalid date, must be %s and match the pattern. Try again.", betweenRequirement),
								TextColor.RED
				);
			}
		} while (true);
	...
	public static LocalDate getDateInputInline(String message, LocalDate minDate, LocalDate maxDate, TextColor textColor) {
		...
		LocalDate date;
		do {
			date = getDateInputInline(
					String.format("%s (between %s and %s)", message, minDate.format(formatter), maxDate.format(formatter)),
					textColor
			);
			if (date.isBefore(minDate) || date.isAfter(maxDate)) {
				date = null;
				String betweenRequirement = String.format("between %s and %s", minDate.format(formatter),
	 ...
	 * 		The message to write to the console
	 * @param textColor
	 * 		The ConsoleTextColor to write the text in
	 */
	 ...
	 * 		The ConsoleBackgroundColor to place behind the text
	 */
	 ...
	 * 		The message to write
	 * @param colors
	 * 		Array of colors to cycle through. <strong>It must be either an array of Color32 or TextColor.</strong>
	 */
	 ...
	 * 		The ConsoleTextColor to write the text in
	 * @param style
	 * 		The TextStyle to apply (BOLD, ITALIC, UNDERLINE, etc.)
	 */
	...
	public static void write(String message, TextColor textColor, BackgroundColor backgroundColor, TextStyle style) {
		System.out.print(
				getTextStyleText(style) + getBackgroundColorText(backgroundColor) + getTextColorText(textColor) + message +
						RESET);
	}
	 ...
	 * 		Delay in milliseconds between each character
	 * @param startingHue
	 * 		Where the hue should start (0-360)
	 * @param rotateHueAmount
	 * 		Amount to rotate the hue by each character
	 */
	 ...
	 * 		Delay in milliseconds between each character
	 * @param colors
	 * 		Array of colors to cycle through (TextColor[] or Color32[])
	 */
	 ...
	 * 		The color of the filled portion
	 * @param showPercent
	 * 		Whether to display the percentage number
	 */
	 ...
	 * 		The width of the progress bar in characters
	 * @param fillColor
	 * 		The Color32 of the filled portion
	 */
	 ...
	 * 		The color of the filled portion
	 * @param emptyColor
	 * 		The color of the empty portion
	 * @param showPercent
	 * 		Whether to display the percentage number
	 */
	 ...
	 * 		The Color32 of the filled portion
	 * @param emptyColor
	 * 		The Color32 of the empty portion
	 * @param showPercent
	 * 		Whether to display the percentage number
	 */
	 ...
	 * 		Whether to display the percentage number
	 */
	public static void writelnProgressBar(int percent, int width, TextColor fillColor, TextColor emptyColor,
										  boolean showPercent
	) {
	 ...
	 * 		The color of the filled portion
	 * @param showValues
	 * 		Whether to display the current/max numbers
	 */
	 ...
	 * 		Whether to display the current/max numbers
	 */
	public static void writeStatBar(int current, int max, int width, TextColor fillColor, TextColor emptyColor,
									boolean showValues
	) {
		if (max <= 0)
			max = 1;
	 ...
	 * 		Whether to display the current/max numbers
	 */
	public static void writelnStatBar(int current, int max, int width, TextColor fillColor, TextColor emptyColor,
									  boolean showValues
	) {
	 ...
	 * 		Whether to display the current/max numbers
	 */
	public static void writelnStatBar(int current, int max, int width, Color32 fillColor, Color32 emptyColor,
									  boolean showValues
	) {
		writeStatBar(current, max, width, fillColor, emptyColor, showValues);
		System.out.println();
	 ...
	 *
	 * @param message
	 * 		The message to display (e.g., "Press Enter to continue...")
	 */
	...
	public static class Color32 {
		 ...
		 *
		 * @param consoleColor
		 * 		Predefined enum values for color
		 *
		 * @see TextColor
		 *
		 */
		 ...
		 *
		 * @param hexcode
		 * 		This will be parsed in the format RRGGBB
		 *
		 */
		 ...
		 * @param lightness
		 * 		Lightness or brightness of the color (0 - 1)
		 *
		 */
		 ...
		 *
		 * @param color
		 * 		Color to convert to hexadecimal
		 *
		 */
		 ...
		 *
		 * @param red
		 * 		Set red value anywhere from 0 to 255
		 *
		 */
		 ...
		 *
		 * @param green
		 * 		Set green value anywhere from 0 to 255
		 *
		 */
		 ...
		 *
		 * @param blue
		 * 		Set blue value anywhere from 0 to 255
		 *
		 */
		 ...
		 *
		 * @param consoleColor
		 * 		Color enum for existing colors
		 *
		 * @see TextColor
		 *
		 */
	 ...
	 * 		The message to write to the console
	 * @param red
	 * 		Amount of red value in the color
	 * @param green
	 * 		Amount of green value in the color
	 * @param blue
	 * 		Amount of blue value in the color
	 *
	 */
	 ...
	 * 		Amount of blue value in the color
	 * @param redBack
	 * 		Amount of red value in the background color
	 * @param greenBack
	 * 		Amount of green value in the background color
	 * @param blueBack
	 * 		Amount of blue value in the background color
	 *
	 */
	public static void write(String message, int red, int green, int blue, int redBack, int greenBack, int blueBack) {
		System.out.print(
				getANSIColorString(red, green, blue, false) + getANSIColorString(redBack, greenBack, blueBack, true) + message +
						RESET);
	}
	...
	public static void write(String message, Color32 textColor, Color32 backgroundColor) {
		System.out.print(getANSIColorString(textColor, false) + getANSIColorString(backgroundColor, true) + message + RESET);
	}
	...
	public static void writeln(String message, Color32 textColor, Color32 backgroundColor) {
		System.out.println(getANSIColorString(textColor, false) + getANSIColorString(backgroundColor, true) + message + RESET);
	}
